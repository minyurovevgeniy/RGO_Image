...
/*
  $testToken="3";
  $studentToken="XvvSust6CwS8PhV8JIpV";
  $answer=1;
  $questionType=1;
  ...
  */
  ...
  $stmt=$pdo->prepare("SELECT * FROM `questions` WHERE `question_test`=? ORDER BY `question_id` DESC");
  $stmt->execute(array($test));
...
// get variants
  ...
  {
    $currentQuestionOrder=$questionsCount;
  }
